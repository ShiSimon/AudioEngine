...
double CAEStreamInfo::GetDuration()
{
  ...
  switch (m_type)
  {
    ...
    case STREAM_TYPE_DTS_2048:
      ...
      break;
    default:
      LOGD("CAEStreamInfo::GetDuration - invalid stream type");
      break;
  }
...
unsigned int CAEStreamParser::SyncAC3(uint8_t *data, unsigned int size)
{
  ...
  for (; size - skip > 7; ++skip, ++data)
  {
    ...
    if (bsid <= 10)
    {
      ...
      LOGD("CAEStreamParser::SyncAC3 - AC3 stream detected (%d channels, %dHz)", m_info.m_channels, m_info.m_sampleRate);
      return skip;
    }
    else
    {
      ...
      LOGD("CAEStreamParser::SyncAC3 - E-AC3 stream detected (%d channels, %dHz)", m_info.m_channels, m_info.m_sampleRate);
      return skip;
    }
  }

  // if we get here, the entire packet is invalid and we have lost sync
  LOGD("CAEStreamParser::SyncAC3 - AC3 sync lost");
  m_hasSync = false;
  m_fsizeMain = 0;
  return skip;
}
...
unsigned int CAEStreamParser::SyncDTS(uint8_t *data, unsigned int size)
{
  ...
  for (; size - skip > 13; ++skip, ++data)
  {
    ...
    if (!m_hasSync || skip || dataType != m_info.m_type || sampleRate != m_info.m_sampleRate || dtsBlocks != m_dtsBlocks)
    {
      ...
      LOGD("CAEStreamParser::SyncDTS - %s stream detected (%d channels, %dHz, %dbit %s, period: %u)",
  ...
  /* lost sync */
  LOGD("CAEStreamParser::SyncDTS - DTS sync lost");
  m_hasSync = false;
  return skip;
}
...
unsigned int CAEStreamParser::SyncTrueHD(uint8_t *data, unsigned int size)
{
  ...
  for (; left; ++skip, ++data, --left)
  {
    ...
    if (syncword == 0xf8726fba)
    {
      ...
      if (!m_hasSync)
        LOGD("CAEStreamParser::SyncTrueHD - TrueHD stream detected (%d channels, %dHz)", m_info.m_channels, m_info.m_sampleRate);
    ...
    else
    {
      ...
      if ((((check >> 4) ^ check) & 0xF) != 0xF)
      {
        /* lost sync */
        m_hasSync  = false;
        LOGD("CAEStreamParser::SyncTrueHD - Sync Lost");
        continue;
      }
      else
      {
