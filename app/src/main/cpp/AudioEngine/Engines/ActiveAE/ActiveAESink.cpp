...
#include "../../Utils/AEBitstreamPacker.h"
...
void CActiveAESink::Stop()
{
  m_sink->Stop();
}

void CActiveAESink::Play()
{
  m_sink->Play();
}


void CActiveAESink::Pause()
{
  m_sink->Pause();
}

void CActiveAESink::Flush()
{
  m_sink->Flush();
}

int CActiveAESink::getPlayState()
{
  return m_sink->getPlayState();
}
...
void CActiveAESink::StateMachine(int signal, Protocol *port, Message *msg)
{
  for (int state = m_state; ; state = SINK_parentStates[state])
  {
    ...
    default: // we are in no state, should not happen
      LOGD("CActiveAESink::%s - no valid state: %d", __FUNCTION__, m_state);
      return;
    }
  } // for
}
...
void CActiveAESink::EnumerateSinkList(bool force)
{
  ...
  {
    LOGD("No Devices found - retry: %d", c_retry);
    usleep(1500*1000);
    c_retry--;
    // retry the enumeration
    CAESinkFactory::EnumerateEx(m_sinkInfoList, true);
  }
  LOGD("Found %lu Lists of Devices", m_sinkInfoList.size());
  PrintSinks();
}
...
void CActiveAESink::PrintSinks()
{
  for (AESinkInfoList::iterator itt = m_sinkInfoList.begin(); itt != m_sinkInfoList.end(); ++itt)
  {
    LOGD("Enumerated %s devices:", itt->m_sinkName.c_str());
    int count = 0;
    for (AEDeviceInfoList::iterator itt2 = itt->m_deviceInfoList.begin(); itt2 != itt->m_deviceInfoList.end(); ++itt2)
    {
      LOGD("    Device %d", ++count);
...
void CActiveAESink::OpenSink(AEAudioFormat &Format, std::string &outdevice)
{
  ...
  LOGD("CActiveAESink::OpenSink - initialize sink");
  ...
  LOGD("CActiveAESink::OpenSink - trying to open device %s", device.c_str());
  ...
  if (!m_sink)
  {
    device = "NULL:NULL";
    m_sinkFormat = m_requestedFormat;
    LOGD("CActiveAESink::OpenSink - open NULL sink");
    m_sink = CAESinkFactory::Create(device, m_sinkFormat, passthrough);
  }

  if (!m_sink)
  {
    LOGD("CActiveAESink::OpenSink - no sink was returned");
    m_extError = true;
    return;
  }
  ...
  LOGD("CActiveAESink::OpenSink - %s Initialized:", m_sink->GetName());
  LOGD("  Output Device : %s", m_deviceFriendlyName.c_str());
  LOGD("  Sample Rate   : %d", m_sinkFormat.m_sampleRate);
  LOGD("  Sample Format : %s", CAEUtil::DataFormatToStr(m_sinkFormat.m_dataFormat));
  LOGD("  Channel Count : %d", m_sinkFormat.m_channelLayout.Count());
  LOGD("  Channel Layout: %s", ((std::string)m_sinkFormat.m_channelLayout).c_str());
  LOGD("  Frames        : %d", m_sinkFormat.m_frames);
  LOGD("  Frame Size    : %d", m_sinkFormat.m_frameSize);
...
unsigned int CActiveAESink::OutputSamples(uint8_t **buffer, uint32_t nb_samples, int32_t pause_burst_ms, AEDelayStatus &outstatus,
                                          int &framesOrPackets_need_to_remove)
{
  //uint8_t **buffer = samples->pkt->data;
  uint8_t *packBuffer;
  unsigned int frames = nb_samples;
  ...
  if (m_requestedFormat.m_dataFormat == AE_FMT_RAW)
  {
    if (m_needIecPack)
    {
      if (frames > 0)
      {
        ...
        if (m_sinkFormat.m_streamInfo.m_type == CAEStreamInfo::STREAM_TYPE_TRUEHD)
        {
          ...
          {
            m_extError = true;
            LOGD("CActiveAESink::OutputSamples - incomplete TrueHD buffer");
            return 0;
          }
        }
        else
    ...
    else
    {
      ...
      if (pause_burst_ms > 0)
      {
        m_sink->AddPause(pause_burst_ms);
  ...
  while (frames > 0)
  {
    ...
    if (written == 0)
    {
      usleep(500*m_sinkFormat.m_frames/m_sinkFormat.m_sampleRate*1000);
      retry++;
      if (retry > 4)
      {
        m_extError = true;
        LOGD("CActiveAESink::OutputSamples - failed");
        status.SetDelay(0);
    ...
    {
      m_extError = true;
      LOGD("CActiveAESink::OutputSamples - sink returned error");
      status.SetDelay(0);
      framesOrPackets = frames;
      if (m_requestedFormat.m_dataFormat == AE_FMT_RAW)
        framesOrPackets = 1;
