...
static jni::CJNIAudioTrack *CreateAudioTrack(int stream, int sampleRate, int channelMask, int encoding, int bufferSize)
{
  ...
  {
    LOGD("AESinkAUDIOTRACK - AudioTrack creation (channelMask 0x%08x): %s", channelMask, e.what());
  }
...
bool CAESinkAUDIOTRACK::Initialize(AEAudioFormat &format, std::string &device)
{
  ...
  m_extTimer.SetExpired();
  LOGD("CAESinkAUDIOTRACK::Initialize requested: sampleRate %u; format: %s; channels: %d", format.m_sampleRate, CAEUtil::DataFormatToStr(format.m_dataFormat), format.m_channelLayout.Count());
  ...
  for (auto& s : m_sink_sampleRates)
  {
     ...
     if (d < distance)
     {
       m_sink_sampleRate = s;
       distance = d;
       LOGD("Updated SampleRate: %u Distance: %u", m_sink_sampleRate, d);
     }
  }
  ...
  if (m_format.m_dataFormat == AE_FMT_RAW)
  {
        ...
        m_format.m_streamInfo.m_type == CAEStreamInfo::STREAM_TYPE_TRUEHD)
    {
      ...
      if (!m_info.m_wantsIECPassthrough && movie_GetSDKVersion() == 22 && m_sink_sampleRate > 48000)
    ...
    if (m_info.m_wantsIECPassthrough)
    {
      ...
      if (m_encoding == -1)
      {
        ...
        m_encoding = CJNIAudioFormat::ENCODING_PCM_16BIT;
        LOGD("Fallback to PCM passthrough mode - this might not work!");
      }
    }
  }
  else
  {
    m_passthrough = false;
    m_format.m_sampleRate     = m_sink_sampleRate;
    if (movie_GetSDKVersion() >= 21 && m_format.m_channelLayout.Count() == 2)
    {
  ...
  while (!m_at_jni)
  {
    LOGD("Trying to open: %u samplerate %d channelMask %d encoding", m_sink_sampleRate, atChannelMask, m_encoding);
    ...
    if (min_buffer < 0)
    {
      LOGD("Minimum Buffer Size was: %d - disable passthrough (?) your hw does not support it", min_buffer);
      return false;
    }

    m_min_buffer_size = (unsigned int) min_buffer;
    LOGD("Minimum size we need for stream: %u", m_min_buffer_size);
    ...
    if (m_passthrough && !m_info.m_wantsIECPassthrough)
    {
      ...
      LOGD("Opening Passthrough RAW Format: %s Sink SampleRate: %u", CAEUtil::StreamTypeToStr(m_format.m_streamInfo.m_type), m_sink_sampleRate);
    ...
    LOGD("Created Audiotrackbuffer with playing time of %lf ms min buffer size: %u bytes",
    ...
    if (!IsInitialized())
    {
      if (!m_passthrough)
      {
        ...
        {
          atChannelMask = CJNIAudioFormat::CHANNEL_OUT_5POINT1;
          LOGD("AESinkAUDIOTRACK - Retrying multichannel playback with a 5.1 layout");
          continue;
        }
        else if (atChannelMask != CJNIAudioFormat::CHANNEL_OUT_STEREO)
        {
          atChannelMask = CJNIAudioFormat::CHANNEL_OUT_STEREO;
          LOGD("AESinkAUDIOTRACK - Retrying with a stereo layout");
          continue;
        }
      }
      LOGD("AESinkAUDIOTRACK - Unable to create AudioTrack");
      Deinitialize();
      return false;
    }
    const char* method = m_passthrough ? (m_info.m_wantsIECPassthrough ? "IEC (PT)" : "RAW (PT)") : "PCM";
    LOGD("CAESinkAUDIOTRACK::Initializing with: m_sampleRate: %u format: %s (AE) method: %s stream-type: %s min_buffer_size: %u m_frames: %u m_frameSize: %u channels: %d",
...
{
#ifdef DEBUG_VERBOSE
  LOGD("CAESinkAUDIOTRACK::Deinitialize");
#endif
  // Restore volume
  if (m_volume != -1)
  {
...
void CAESinkAUDIOTRACK::GetDelay(AEDelayStatus& status)
{
  ...
  {
    LOGD("You did it wrong man - fully wrong! offset %lld head pos %llu", m_offset, m_headPos);
    m_offset = 0;
  }
...
unsigned int CAESinkAUDIOTRACK::AddPackets(uint8_t **data, unsigned int frames, unsigned int offset)
{
  ...
  if (frames)
  {
    ...
    while (written < size)
    {
      ...
      if (loop_written < 0)
      {
        LOGD("CAESinkAUDIOTRACK::AddPackets write returned error:  %d", loop_written);
        return INT_MAX;
      }
      ...
      if (loop_written == 0)
      {
        if (!retried)
        {
          ...
          LOGD("Retried to write onto the sink - slept: %lf playing: %s", sleep_time, playing ? "yes" : "no");
          continue;
        }
        else
        {
          LOGD("Repeatedly tried to write onto the sink - giving up");
          break;
        }
      }
      ...
      if (m_passthrough && !m_info.m_wantsIECPassthrough)
      {
        ...
        {
          LOGD("Error writing full package to sink, left: %d", size_left);
...
void CAESinkAUDIOTRACK::EnumerateDevicesEx(AEDeviceInfoList &list, bool force)
{
  ...
  if (1)
  {
    ...
    if (CJNIAudioFormat::ENCODING_AC3 != -1)
    {
      ...
      LOGD("Firmware implements AC3 RAW");
    }
    ...
    if (CJNIAudioFormat::ENCODING_E_AC3 != -1)
    {
      ...
      LOGD("Firmware implements EAC3 RAW");
    }

    if (CJNIAudioFormat::ENCODING_DTS != -1)
    {
      LOGD("Firmware implements DTS RAW");
    ...
    if (aml_present() && movie_GetSDKVersion() < 23)
    {
      // passthrough
      m_info.m_wantsIECPassthrough = true;
    ...
    else
    {
      ...
      for (int i=0; i<test_sample_sz; ++i)
      {
        if (IsSupported(test_sample[i], CJNIAudioFormat::CHANNEL_OUT_STEREO, encoding))
        {
          ...
          LOGD("AESinkAUDIOTRACK - %d supported", test_sample[i]);
        }
      }
      if (movie_GetSDKVersion() >= 23)
      {
        if (CJNIAudioFormat::ENCODING_DTS_HD != -1)
        {
          LOGD("Firmware implements DTS-HD RAW");
          m_info.m_streamTypes.push_back(CAEStreamInfo::STREAM_TYPE_DTSHD);
        }
        if (CJNIAudioFormat::ENCODING_DOLBY_TRUEHD != -1)
        {
          LOGD("Firmware implements TrueHD RAW");
          m_info.m_streamTypes.push_back(CAEStreamInfo::STREAM_TYPE_TRUEHD);
        }
      }
      ...
      if (CJNIAudioFormat::ENCODING_IEC61937 != -1)
      {
        ...
        m_info.m_streamTypes.push_back(CAEStreamInfo::STREAM_TYPE_DTS_2048);
        m_info.m_streamTypes.push_back(CAEStreamInfo::STREAM_TYPE_DTS_512);
        LOGD("AESinkAUDIOTrack: Using IEC PT mode: %d", CJNIAudioFormat::ENCODING_IEC61937);
        ...
        if (supports_192khz)
        {
          ...
          if (IsSupported(192000, atChannelMask, CJNIAudioFormat::ENCODING_IEC61937))
          {
            m_info.m_streamTypes.push_back(CAEStreamInfo::STREAM_TYPE_DTSHD);
            m_info.m_streamTypes.push_back(CAEStreamInfo::STREAM_TYPE_TRUEHD);
            LOGD("8 Channel PT via IEC61937 is supported");
          }
        }
      }
    }
