...
CJNIContext::CJNIContext(jobject context)
{
...
*/
jobject CJNIContext::getSystemService(const std::string service)
{
  JNIEnv *env = movie_jnienv();
  if(env <=0 || CJNIContext::m_context == 0)
    return 0;

  jclass clazz = env->FindClass("android/content/Context");
  jmethodID getSystemServiceID = env->GetMethodID(clazz, "getSystemService", "(Ljava/lang/String;)Ljava/lang/Object;");
  env->DeleteLocalRef(clazz);
  jstring jservicestr=env->NewStringUTF(service.c_str());
  jobject jservice = env->CallObjectMethod(CJNIContext::m_context,getSystemServiceID,jservicestr);
